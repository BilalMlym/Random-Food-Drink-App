...
class DrinkClientTest(unittest.TestCase):
    def setUp(self):
        self.drink_client = DrinkClient()

    def test_get_categories(self):
        mock = MagicMock(return_value='dummy_response')
        self.drink_client.make_request = mock
        res = self.drink_client.get_categories()
    ...
    def test_get_random_drink(self):
        ...
        self.drink_client.make_request = request_mock

        random_mock = MagicMock(return_value=1)
        random.randint = random_mock

        res = self.drink_client.get_random_drink('dummy_category')
